...
def main() -> None:
    ...
    for i, (rel, caption, blurb) in enumerate(ANNEXES, start=1):
        ...
        total += n
        bodies.append((i, caption, blurb, code))
    ...
    print(f"\nstandalone: {OUT.name}  ({OUT.stat().st_size/1024:.0f} KB)")

    build_merged(bodies)


# --------------------------------------------------------------------------
# the same annexes, appended to a copy of the thesis
# --------------------------------------------------------------------------
THESIS = OUT.parent / "Lupu_Silviu-George_COMPLET_cap1-4.tex"
MERGED = OUT.parent / "Lupu_Silviu-George_COMPLET_cap1-4_cu_anexe.tex"

# Loaded after the packages the thesis already has. `setspace` is already in
# use by the class -- \setstretch{1.2} appears in the preamble -- so the
# annexes can be set to single spacing locally without touching the rest.
MERGE_PACKAGES = r"""
% ---------- Code listings, for the annexes ----------
\usepackage{listings}
\usepackage{xcolor}
...
def build_merged(bodies: list[tuple[int, str, str, str]]) -> None:
    """Write a copy of the thesis with the annexes appended.

    The original is never modified. The copy differs from it in exactly two
    places: the two packages the listings need, and the annexes themselves,
    inserted after the bibliography.
    """
    if not THESIS.is_file():
        print(f"  (thesis not found at {THESIS}; merged file not built)")
        return
    tex = THESIS.read_text(encoding="utf-8")

    anchor = "\\usepackage{placeins}"
    if anchor not in tex:
        raise SystemExit(f"anchor {anchor!r} not found in the thesis preamble")
    tex = tex.replace(anchor, anchor + "\n" + MERGE_PACKAGES, 1)

    out = ["\n% =========================================================\n"
           "% Annexes -- generated by chapter4_revision/make_annexes.py\n"
           "% Do not edit here; edit the source files and regenerate.\n"
           "% =========================================================\n\n"
           "\\begin{spacing}{1}\n\n"]
    ...
    out.append("\\end{spacing}\n\n")

    end = "\\end{document}"
    if tex.count(end) != 1:
        raise SystemExit(f"expected exactly one {end!r} in the thesis")
    tex = tex.replace(end, "".join(out) + end, 1)

    MERGED.write_text(tex, encoding="utf-8")
    print(f"complet   : {MERGED.name}  ({MERGED.stat().st_size/1024:.0f} KB)")
    print(f"original   : {THESIS.name}  -- neatins")
